      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card-elevated p-5">
          ...
          <div className="mono text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">
            +LKR {Number(flowData?.totalIncome || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </div>
        ...
        <div className="card-elevated p-5">
          ...
          <div className="mono text-2xl font-bold text-destructive mt-1">
            -LKR {Number(flowData?.totalExpense || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </div>
        ...
        <div className="card-elevated p-5">
          ...
          <div className={`mono text-2xl font-bold mt-1 ${
            ...
            LKR {Number(flowData?.netSurplus || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </div>
          ...
          {flowData?.weeklyData?.map((w: any) => {
            ...
            return (
              <div key={w.week} className="p-2.5 rounded-lg bg-card border border-border text-center space-y-1">
                ...
                <div className="mono text-[11px] font-bold text-emerald-600 dark:text-emerald-400">+{Number(w.income).toLocaleString()}</div>
                <div className="mono text-[11px] font-bold text-destructive">-{Number(w.expense).toLocaleString()}</div>
                <div className={`mono text-[10px] font-bold pt-1 border-t border-border ${isPositive ? 'text-primary' : 'text-destructive'}`}>
                  {isPositive ? '+' : ''}{Number(w.net).toLocaleString()}
                </div>
              </div>
            );
          })}
                ...
                {filteredExpenses.map(exp => (
                  <tr key={exp.id}>
                    ...
                    <td className="text-right mono font-bold text-xs text-destructive">
                      -LKR {Number(exp.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </td>
